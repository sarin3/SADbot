...
async def curse(ctx, com = None, cword = None):
    ...
    elif com.lower == "del" or com.lower() == "delete":
        curses.remove(cword)
    elif com.lower == "dis" or com.lower() == "display":
        # await ctx.send("There are " + curses.count() + "curses")
        for word in curses:
            await ctx.send(word)
    elif com == None and cword == None:
        await ctx.send("Invalid curse command")

@bot.command(pass_context=True)
@commands.has_role("Admin") 
async def giverole(ctx, user: discord.Member, role: discord.Role):
    if not get(ctx.guild.roles, role):
        await ctx.send(f"hey @{ctx.author.name}, this role does not exist. Try !addrole to create a new role!")
